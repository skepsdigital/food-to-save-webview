...
@Component({
  selector: 'app-blip-ticket',
  templateUrl: './blip-ticket.component.html',
  styleUrls: ['./blip-ticket.component.scss']
})
export class BlipTicketComponent implements OnInit {
  ...
  async transferTicketToAgent(ticketId: any, event: any, team: any) {
    ...
    let result = await this.http.post<any>('https://foodtosave.http.msging.net/commands', {
        ...
        headers: {
          'Authorization': "Key YmxpcHRpY2tldDE6OTVIcVhlc2Z3a0hlS01hc0xVSDM=" // Hardcoded API key - consider moving to environment variables
        }
      }).toPromise();
  ...
  async listAttendants() {
    let response = await this.http.post<any>('https://foodtosave.http.msging.net/commands', {
      ...
      "id": uuidv4()
        ...
        headers: {
          'Authorization': "Key YmxpcHRpY2tldDE6OTVIcVhlc2Z3a0hlS01hc0xVSDM="
        }
      }).toPromise();
